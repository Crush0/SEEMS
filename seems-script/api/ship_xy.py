...
def format_query_string(data):
    ...
    if isinstance(data, list):
        return "jsonArray=" + json.dumps(data, separators=(',', ':'))
    ...
    for k in keys:
        ...
        if isinstance(v, (list, dict)):
            # JS 原代码逻辑中此处有点奇怪：[c, JSON.stringify(a)]，a 是外层大对象
            # 但通常这种签名的意图是序列化当前值，这里严格遵循原 JS 逻辑：
...
response = requests.post('https://www.shipxy.com/ship/GetShipm', cookies=cookies, headers=headers, data=data)
print(response.json())
# {'status': 0, 'data': [{'source': 0, 'mmsi': '413541070', 'shipid': '4AC48883A371F196', 'tradetype': 99, 'type': 52, 'imo': '0', 'name': 'YUNGANGDIANTUOERHAO', 'matchtype': 0, 'cnname': '', 'callsign': '0', 'length': 380, 'width': 120, 'left': 60, 'trail': 100, 'draught': 4000, 'dest': 'LIANYUNGANG,CN', 'aisdest': 'LIANYUNGANG      ', 'eta': '03-09 14:00', 'laststa': 1773038230, 'lon': 119439400, 'lat': 34749900, 'sog': 3344, 'cog': 9310, 'hdg': 9300, 'rot': 0, 'navistatus': 0, 'lastdyn': 1773038555, 'satelliteutc': 0}]}
# sog / 514 为真正的航速 lon 和 lat要除以1e6才是经纬度